...
# health will be one of these values: 
  if [[ ! $health =~ "starting" ]]; then
    ...
    if [[ $checked != 1 ]]; then
        ...
        pytest -v $CI_PROJECT_DIR/tests/test_savedsearches.py --splunk-type=external --splunk-app=$CI_PROJECT_DIR/package/ --splunk-data-generator=$CI_PROJECT_DIR/tests/knowledge/pytest-splunk-addon-data.conf --splunk-host=$my_cont_ip --splunk-port=8089 --splunk-user=$USER --splunk-password=$PASSWORD --splunk-hec-token=$HEC_TOKEN
