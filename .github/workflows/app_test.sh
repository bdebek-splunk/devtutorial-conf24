  ...
  if [[ ! $health =~ "starting" ]]; then
    ...
    if [[ $checked != 1 ]]; then
        ...
        if ! docker exec -i -u splunk $CONTAINER_NAME bash -c "SPLUNK_USERNAME=$USER SPLUNK_PASSWORD=$PASSWORD /opt/splunk/bin/splunk search '| rest /servicesNS/-/-/saved/searches | table title'" | grep -q "Movies By Rating"; then
            ...
            pytest $CI_PROJECT_DIR/tests/test_savedsearches.py --splunk-type=external --splunk-app=$CONTAINER_NAME:$APPS_DIR/$APP_ROOT --splunk-data-generator=pytest-splunk-addon-data.conf --splunk-host=$my_cont_ip --splunk-port=8089 --splunk-user=$USER --splunk-password=$PASSWORD --splunk-hec-token=new-token
